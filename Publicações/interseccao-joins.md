...
### 5. SELF JOIN: Hierarquias

Embora sua estrutura atual não tenha hierarquia interna em `tb_clientes`, o `SELF JOIN` é usado quando uma tabela referencia ela mesma. Se você tivesse uma coluna `indicado_por` na `tb_clientes`, faria assim:

```sql
SELECT c.nome AS cliente, i.nome AS quem_indicou
FROM clientes c
LEFT JOIN clientes i ON c.indicado_por = i.id;

```

---

### Dicas de Ouro do seu material:
...
Se você quiser deixar esse relatório mais completo, exibindo também o nome do cliente que fez o pedido, basta adicionar a tabela `clientes` ao `JOIN`:
